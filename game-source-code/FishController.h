...
/*!
Behaves similarily to EnemyController. Class capable of creating, updating and reading a fish row (and hence each individual fish
as well) that exists.
*/
...
class FishController
{
    public:
        ...
        void removeFish(unsigned int fishNum); /*!< Removes the specified fish from the row of existing fish.
                                                    The input integer must be less than or equal to the number of created fish. */
        void update(float deltaTime); /*!< Update the positions of all fish within a row respective to the parameter. If no
                                           row of fish exists, create a new row of fish. */
        void draw(sf::RenderWindow& window); /*!< Draw all fish within a row of fish with reference to a RenderWindow. */
    private:
        void newFishRow();
